          ...
          >
            <div className="my-12 inline-block w-full max-w-3xl transform overflow-hidden rounded border border-gray-400/30 text-left shadow-xl transition-all">
              ...
              >
                ...
                {results.error && (
                  <div className="px-4 py-12 text-center text-sm font-medium">{`搜索失败：${results.error.message}`}</div>
                )}
                {results.result && (
                  <>
                    {results.result.length === 0 ? (
                      <div className="px-4 py-12 text-center text-sm font-medium">{'没有找到相关内容。'}</div>
                    ) : (
